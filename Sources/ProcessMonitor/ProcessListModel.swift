...
final class ProcessListModel: ObservableObject {
    ...
    @Published var sortOrder: [KeyPathComparator<ProcessEntry>] = [KeyPathComparator(\.cpu, order: .reverse)]
    ...
    var filteredEntries: [ProcessEntry] {
        ...
        let matching: [ProcessEntry] = if trimmed.isEmpty {
            entries
        } else if let pid = pid_t(trimmed) {
            entries.filter { $0.id == pid || $0.name.localizedCaseInsensitiveContains(trimmed) }
        } else {
            entries.filter {
