    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text("Sampling only happens while the panel is open.")
                .font(.caption)
                .foregroundStyle(.secondary)
            Toggle("Group processes with their child processes", isOn: $settings.groupByParent)
                .toggleStyle(.checkbox)
            Text("Each app is shown as one row with the combined usage of its helpers. Click ▸ to expand it.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            Divider()
